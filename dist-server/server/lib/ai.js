...
function buildBriefInteractionSummary(patterns) {
...
function buildBriefTensions(patterns) {
...
export function buildPatternsBrief(patterns) {
    ...
    if (!sourcePatterns.length)
        return null;
    const currentState = dedupePatternLines(sourcePatterns.map((pattern) => buildBriefWhyNow(pattern))).slice(0, 3);
    ...
    const summary = sourcePatterns.length >= 3
