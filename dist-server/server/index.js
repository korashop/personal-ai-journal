...
function shouldRefreshPatterns(entriesCount, patterns) {
    ...
    if (entriesCount >= 10 && patterns.length <= 5)
        return true;
    if (patterns.some((pattern) => pattern.rankScore == null))
        return true;
    const promotedSingletonCount = patterns.filter((pattern) => (pattern.entryCount ?? 0) <= 1 &&
        pattern.prominence !== 'quiet').length;
    if (patterns.length >= 5 && promotedSingletonCount / patterns.length >= 0.4)
        return true;
    ...
    if (patterns.length >= 5 && genericQuestionCount / patterns.length >= 0.6)
        return true;
    if (patterns.some((pattern) => !pattern.dimensions.length))
        return true;
...
async function handleCompanionReply(request, response, next) {
    try {
