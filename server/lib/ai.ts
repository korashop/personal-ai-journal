...
function keywordHitCount(text: string, patterns: RegExp[]) {
...
function familyCueScore(familyKey: string, text: string) {
  const cueSets: Record<string, RegExp[]> = {
...
function familiesConflict(leftKey?: string | null, rightKey?: string | null) {
...
function themeFamilyForText(text: string) {
  ...
  const scoredFamilies = THEME_FAMILIES
    .map((family) => {
      let score = 0
      if (family.test.test(cleaned)) score += 5
  ...
  if (second && best.score - second.score < 0.55 && familiesConflict(best.family.key, second.family.key)) {
    ...
    if (secondCue > bestCue) return second.family
  }
...
function uncategorizedThemeTitleLooksTooGeneric(title: string) {
...
function scoreEvidenceLine(line: string, family: ThemeFamily) {
  const clean = normalizeWhitespace(stripMarkdown(cleanTruncatedEnding(line)))
  if (!clean || !family.test.test(clean) || evidenceLooksFragmentary(clean)) return -1
  let score = 0
  const wordCount = clean.split(' ').filter(Boolean).length
  if (wordCount >= 8 && wordCount <= 32) score += 3
  if (wordCount < 6) score -= 4
  if (/[.!?]$/.test(clean)) score += 2
  else score -= 1
  if (/^(what|why|how)\b/i.test(clean)) score -= 1
  ...
  if (/\b(?:through authentic alignment|same pattern|same idea)\b/i.test(clean)) score -= 4
  return score
}

function bestFamilyEvidenceLine(sourceLines: string[], family: ThemeFamily) {
  return sourceLines
    .map((line) => ({
...
function sourceLinesForPatternEvidence(entry: JournalEntry) {
  return [
...
function scoreThreadMatchToPattern(pattern: PatternSection, thread: EntryThread) {
  ...
  const familyPenalty =
    patternFamily?.key && threadFamily?.key && familiesConflict(patternFamily.key, threadFamily.key)
      ? 0.38
...
function buildOverviewFromCluster(
  ...
  const familyOverviews: Record<string, string> = {
    ...
    'output-anchor':
      'Shipped output keeps appearing as an anchor for meaning: you want days to feel defined by making something real, and you notice how consuming or circling ideas can become a substitute.',
    ...
    'missed-window':
      'A recurring story here is about missed timing: replaying earlier moments when you did not act, and trying to separate useful signal from a self-punishing sense that a window has already closed.',
  }

  const intro =
    (familyKey ? familyOverviews[familyKey] : '') ||
    (entryCount >= 2
...
function scoreThemeCoherence(title: string, overview: string, dimensions: string[]) {
  ...
  dimensions.forEach((left, leftIndex) => {
    dimensions.slice(leftIndex + 1).forEach((right) => {
      ...
      if (score > 0) pairScores.push(score)
    })
  })
  ...
  if (coherence >= 0.45) return 5
  if (coherence >= 0.3) return 3
  if (coherence >= 0.18) return 1
  return 0
}

function scoreThemeCharge(text: string) {
  const clean = normalizeWhitespace(stripMarkdown(cleanTruncatedEnding(text)))
  let score = 0
...
function scoreQuestionUsefulness(questions: string[]) {
  return questions.reduce((sum, question) => {
    let score = 0
    if (/\bwhat|how|where\b/i.test(question)) score += 1
    if (/\btest|move|shift|require|reveal|cost|different|concrete\b/i.test(question)) score += 1
...
function scoreThemeStatus(status?: PatternSection['status']) {
...
function scoreThemeSignal(
  ...
  const chargeScore = Math.min(
    8,
    scoreThemeCharge(pattern.overview) +
...
function stripPatternIdentity(
  pattern: PatternSection,
): Omit<PatternSection, 'id' | 'updatedAt' | 'entryCount' | 'status'> {
  return {
    title: pattern.title,
...
function patternsReferToSameTheme(
...
function mergeEnrichedWithFallbackPatterns(
  enrichedPatterns: Array<Omit<PatternSection, 'id' | 'updatedAt' | 'entryCount' | 'status'>>,
  fallbackPatterns: PatternSection[],
) {
    ...
    if (scoreThemeSignal(fallback) > scoreThemeSignal(existing)) {
      merged[duplicateIndex] = {
        ...
        exploreOptions: dedupePatternLines(
          [...existing.exploreOptions, ...fallback.exploreOptions],
        ).slice(0, 3),
...
export function attachPatternSupportingEvidence(
  patterns: PatternSection[],
  entries: JournalEntry[],
): PatternSection[] {
  const entryMap = new Map(entries.map((entry) => [entry.id, entry]))
  ...
  return patterns.map((pattern) => {
    ...
    const dedupedSnippets = dedupePatternLines(
      derivedEvidence.map((item) => item.snippet),
      pattern.overview,
    )
...
export async function generatePatternsUpdate(
  patterns: PatternSection[],
  entries: JournalEntry[],
  memoryDoc: MemoryDocumentRecord | null,
  userMessage?: string,
...
Recent entries:
${recentEntriesForPrompt(
  entries.map((entry) => ({ ...entry, summary: `${entry.title}: ${entry.summary}` })),
  6,
  220,
)}
...
function summarizePatternLifeAreas(patterns: PatternSection[]) {
  ...
  if (!counts.size) return 'No clear area breakdown yet.'
...
function classifyPatternLifeArea(pattern: PatternSection) {
  const familyKey = patternFamilyKey(pattern)
